  ...
  return (
    <div className="min-h-screen bg-bg text-text">
      <div className="max-w-4xl mx-auto px-6 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-2">{t('legal.refunds.title')}</h1>
          <p className="text-muted">{t('legal.updated', { date: UPDATED_AT })}</p>
        </div>
        <div className="prose prose-invert max-w-none space-y-6">
          <p className="text-muted leading-relaxed">{t('legal.refunds.intro')}</p>
          {SECTIONS.map((k) => (
            <section key={k}>
              <h2 className="text-2xl font-semibold mb-3">{t(`legal.refunds.${k}Title`)}</h2>
              <p className="text-muted leading-relaxed">{t(`legal.refunds.${k}Body`)}</p>
            </section>
          ))}
          <section>
            <h2 className="text-2xl font-semibold mb-3">{t('legal.refunds.contactTitle')}</h2>
