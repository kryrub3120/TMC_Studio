...
export function usePublicLightTheme() {
  useEffect(() => {
    ...
    body.classList.add('bg-white', 'text-slate-950');

    return () => {
      if (hadDark) html.classList.add('dark');
      else html.classList.remove('dark');
      html.style.colorScheme = previousColorScheme;
      body.classList.remove('bg-white', 'text-slate-950');
    };
  }, []);
}
...
export function PublicPageShell({ title, description, updatedAt, children }: PublicPageShellProps) {
  ...
  return (
    <div className="min-h-screen bg-[#F6F8FC] font-sans text-slate-950">
      <header className="sticky top-0 z-topbar border-b border-slate-200 bg-white/85 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3">
          <Link to="/" className="text-lg font-bold tracking-tight text-slate-950">TMC&nbsp;Studio</Link>
          <nav className="hidden items-center gap-5 md:flex" aria-label="Public navigation">
            ...
            <Link to="/privacy" className="text-sm font-medium text-slate-600 hover:text-slate-950">
              {t('footer.privacy')}
            </Link>
            <Link to="/terms" className="text-sm font-medium text-slate-600 hover:text-slate-950">
              {t('footer.terms')}
            </Link>
          </nav>
      ...
      <main className="mx-auto max-w-4xl px-4 py-10 sm:px-6 sm:py-14">
        <div className="mb-8 border-b border-slate-200 pb-8">
          <Link to="/" className="mb-6 inline-flex items-center gap-2 text-sm font-medium text-slate-600 hover:text-slate-950">
          ...
          {updatedAt && <p className="mt-4 text-sm text-slate-500">{t('legal.updated', { date: updatedAt })}</p>}
        </div>
...
function CompanyPanel() {
  return (
    <aside className="mt-10 rounded-lg border border-slate-200 bg-white p-5 text-sm leading-6 text-slate-600 shadow-sm">
      <p className="font-semibold text-slate-950">{COMPANY_DETAILS.displayName}</p>
