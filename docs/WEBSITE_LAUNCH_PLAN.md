...
- **CTA główne:** „Open the board — no signup" → `/app` (guest mode).
...
Już istnieje (pobiera release z GitHub). Dodać: detekcję OS (podświetl właściwy przycisk), wymagania systemowe, „dlaczego desktop" (offline, szybkość, brak zakładek), link „lub użyj w przeglądarce" → `/app`.
...
- Wydzielić `/` (marketing, SSG/statyczny, lekki) od `/app` (dzisiejszy edytor). Edytor leniwie ładowany — landing nie ciągnie Konvy/edytora.
- Router: marketing `BrowserRouter` + prefiks języka; desktop (Tauri) `HashRouter` startuje wprost w `/app`.
...
- **Blog/SEO (`/blog`):** tutoriale i „evergreen" pod frazy: „football formations explained", „4-3-3 vs 4-2-3-1", „how to make tactic animations", „free tactics board". Każdy artykuł = CTA do `/app`.
...
|---|---|---|
| 1.1 | Rozdzielić `/` (landing) i `/app` (edytor) | Landing nie ładuje bundla edytora; guest wchodzi na `/app` jednym klikiem |
...
2. **Rozdział `/` i `/app`:** tak — landing oddzielony od edytora.
