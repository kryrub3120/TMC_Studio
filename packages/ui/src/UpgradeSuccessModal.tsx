...
interface UpgradeSuccessModalProps {
  ...
  onClose: () => void;
  plan: 'pro' | 'team';
  mode?: 'activating' | 'success'; // activating = loading, success = celebration
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#1a1a2e] rounded-xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden border border-white/10">
        ...
        <div className="relative p-8 text-center">
          {/* Icon */}
          <div className={`inline-flex items-center justify-center w-20 h-20 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 shadow-lg shadow-blue-500/30 mb-6 ${isActivating ? 'animate-spin' : 'animate-bounce'}`}>
            <span className="text-4xl">{isActivating ? '⏳' : '🎉'}</span>
          </div>

          {/* Title */}
          <h2 className="text-2xl font-bold text-white mb-2">
            {isActivating ? 'Activating your subscription...' : `Welcome to ${planName}!`}
          </h2>
          <p className="text-gray-400 mb-6">
            {isActivating 
              ? 'This can take a few seconds. You can keep using the app.' 
              : 'Your account has been upgraded successfully'}
          </p>

          {/* Features Grid (only on success) */}
          {!isActivating && (
            <>
