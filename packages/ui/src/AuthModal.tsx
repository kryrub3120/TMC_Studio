...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const googleInProgress =
    oauthStatus === 'oauthOpening' ||
    oauthStatus === 'oauthWaitingForProvider' ||
    oauthStatus === 'oauthCallbackReceived' ||
    oauthStatus === 'sessionReady' ||
    oauthStatus === 'profileHydrating';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-[#1a1a2e] rounded-xl shadow-2xl w-full max-w-md mx-4 overflow-hidden border border-white/10">
        ...
        <div className="p-6">
          ...
          {googleInProgress && mode !== 'forgot' && (
            <div className="mb-5 rounded-lg border border-blue-500/40 bg-blue-500/10 p-4 text-sm text-blue-100">
              <div className="flex items-start gap-3">
                <div className="mt-0.5 h-5 w-5 shrink-0 rounded-full border-2 border-blue-300/30 border-t-blue-300 animate-spin" />
                <div>
                  <p className="font-semibold text-white">{t('auth.googleProgressTitle')}</p>
                  <p className="mt-1 leading-5 text-blue-100/80">{t('auth.googleProgressDescription')}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {mode !== 'forgot' && onSignInWithGoogle && (
            <>
              <button
                ...
                disabled={isLoading || googleInProgress}
