...
vi.mock('stripe', () => {
  ...
  return {
    ...
    __mockSubscriptionsRetrieve: mockSubscriptionsRetrieve,
    __mockCustomersRetrieve: mockCustomersRetrieve,
  };
});
...
describe('Checkout Security (create-checkout.ts)', () => {
  ...
  it('propagates yearly priceId with correct billing_cycle metadata', async () => {
    const stripeModule = await import('stripe');
    const mockCreate = (stripeModule as any).__mockCheckoutSessionsCreate;
    mockCreate.mockClear();
    ...
    const event = makeEvent({
      body: { priceId: 'price_1Sr4JVANogcZdSR3locOvXlL' }, // pro yearly
    });
    const res = await handler(event, {} as any);
    expect(res.statusCode).toBe(200);

    const callArgs = mockCreate.mock.lastCall[0];
    expect(callArgs.line_items[0].price).toBe('price_1Sr4JVANogcZdSR3locOvXlL');
    expect(callArgs.subscription_data.metadata.plan).toBe('pro');
    expect(callArgs.subscription_data.metadata.billing_cycle).toBe('yearly');
  });

  it('uses billingCycle from body over priceId derivation', async () => {
    const stripeModule = await import('stripe');
    const mockCreate = (stripeModule as any).__mockCheckoutSessionsCreate;
    mockCreate.mockClear();
    mockCreate.mockResolvedValue({ id: 'cs_test_789', url: 'https://checkout.stripe.com/test' });

    // Pass body.billingCycle explicitly — even though priceId is monthly,
    // the body value should win.
    const event = makeEvent({
      body: {
        priceId: 'price_1Sr4E7ANogcZdSR3Dwu2aPbV', // pro monthly
        billingCycle: 'yearly', // override
      },
    });
    const res = await handler(event, {} as any);
    expect(res.statusCode).toBe(200);
...
describe('Stripe Config Consistency', () => {
  ...
  it('PricingModal config has matching price IDs', async () => {
    const fs = await import('fs');
    const path = await import('path');
    ...
    // All pricing config IDs should exist in backend
    for (const id of pricingIds) {
      expect(backendIds.has(id)).toBe(true);
    }

    // All backend IDs should exist in pricing config
    for (const id of backendIds) {
  ...
  it('getCycleFromPriceId returns correct cycle for all price IDs', async () => {
    const backend = await import('../_stripeConfig');

    expect(backend.getCycleFromPriceId(backend.STRIPE_PRICES.pro.monthly)).toBe('monthly');
    expect(backend.getCycleFromPriceId(backend.STRIPE_PRICES.pro.yearly)).toBe('yearly');
    expect(backend.getCycleFromPriceId(backend.STRIPE_PRICES.team.monthly)).toBe('monthly');
    expect(backend.getCycleFromPriceId(backend.STRIPE_PRICES.team.yearly)).toBe('yearly');

    // Unknown ID returns 'monthly' (safe default)
    expect(backend.getCycleFromPriceId('price_unknown')).toBe('monthly');
  });

  it('PRICE_TO_CYCLE covers all defined price IDs', async () => {
    const backend = await import('../_stripeConfig');
    ...
    for (const id of allIds) {
      expect(backend.PRICE_TO_CYCLE[id]).toBeDefined();
    }
  });
});
...
describe('Webhook Security (stripe-webhook.ts)', () => {
  let handler: typeof import('../stripe-webhook').handler;

  // Fake signed event data for webhook tests
  const fakeEventData = JSON.stringify({
    id: 'evt_test_001',
    type: 'checkout.session.completed',
    data: {
      object: {
        id: 'cs_test_001',
        customer: 'cus_mocked_123',
        subscription: 'sub_mocked_001',
  ...
  /**
   * Build a minimal subscription object for stripe.subscriptions.retrieve mock
   */
    ...
    return {
      ...
      items: {
        data: [
          {
            price: {
              id: 'price_1Sr4E7ANogcZdSR3Dwu2aPbV', // pro monthly
      ...
      start_date: Math.floor(Date.now() / 1000) - 1,
      ...overrides,
    };
  }
  ...
  beforeEach(async () => {
    ...
    // Default: valid auth for supabase
    (__mockSupabase.auth.getUser as any).mockResolvedValue({
      data: { user: { id: 'user-123', email: 'test@example.com' } },
      error: null,
    });

    // Default: constructEvent parses event from body dynamically
    (stripeModule as any).__mockConstructEvent.mockImplementation((body: string) => {
      return JSON.parse(body);
    });

    // Mock stripe.subscriptions.retrieve
    (stripeModule as any).__mockSubscriptionsRetrieve.mockResolvedValue(buildSubscription());

    // Mock stripe.customers.retrieve
    (stripeModule as any).__mockCustomersRetrieve.mockResolvedValue({
      id: 'cus_mocked_123',
      deleted: false,
      email: 'test@example.com',
    });

    // Default from mock that handles both stripe_webhook_events and profiles
    const mockSingleChain = vi.fn().mockResolvedValue({
      data: { id: 'user-123', stripe_customer_id: 'cus_mocked_123' },
      error: null,
    });
    // For claimEvent - return success on first insert
    (__mockSupabase.from as any).mockImplementation((table: string) => {
  ...
  function makeEvent(overrides: Record<string, any> = {}) {
    const { body: overrideBody, ...restOverrides } = overrides;
    return {
      httpMethod: overrides.httpMethod ?? 'POST',
      headers: {
      ...
      body: overrideBody ?? fakeEventData,
      ...restOverrides,
    } as any;
  }
  ...
  it('returns 500 when env vars are missing', async () => {
    ...
    // Need to re-import after env change - but since handler captures
    // module-level vars, we need a fresh import
    vi.resetModules();
    process.env.SUPABASE_URL = 'https://test.supabase.co';
    process.env.SUPABASE_SERVICE_ROLE_KEY = 'test-service-role-key';
    process.env.STRIPE_SECRET_KEY = 'sk_test_mock';
    // STRIPE_WEBHOOK_SECRET intentionally missing
    const mod = await import('../stripe-webhook');
    const h = mod.handler;
    const event = makeEvent({});
  ...
  it('returns 400 when stripe-signature header is missing', async () => {
    const event = makeEvent({ headers: { 'x-forwarded-for': '127.0.0.1' } });
  ...
  it('returns 400 when webhook signature verification fails', async () => {
    const stripeModule = await import('stripe');
    (stripeModule as any).__mockConstructEvent.mockImplementation(() => {
  ...
  it('returns 200 with duplicate:true for already-processed event (idempotency)', async () => {
    const { __mockSupabase } = await import('@supabase/supabase-js');
    // First call to stripe_webhook_events.insert succeeds (first event)
    ...
    (__mockSupabase.from as any).mockImplementation((table: string) => {
      ...
      // For profiles — return a user
      return {
        select: vi.fn().mockReturnThis(),
        eq: vi.fn().mockReturnThis(),
        single: vi.fn().mockResolvedValue({ data: { id: 'user-123', stripe_customer_id: 'cus_mocked_123' }, error: null }),
        insert: vi.fn().mockReturnThis(),
        update: vi.fn().mockReturnThis(),
      };
    });
    ...
    // Second identical event (simulate Stripe retry)
    callCount = 1; // Reset so the insert mock will detect duplicate
  ...
  it('processes checkout.session.completed and upgrades user profile', async () => {
    const { __mockSupabase } = await import('@supabase/supabase-js');
    ...
    const stripeModule = await import('stripe');
    (stripeModule as any).__mockSubscriptionsRetrieve.mockResolvedValue(
      buildSubscription({
        items: {
          data: [
            {
  ...
  it('processes customer.subscription.updated and flips tier', async () => {
    const { __mockSupabase } = await import('@supabase/supabase-js');
      ...
      buildSubscription({
        items: {
          data: [
            {
              price: {
                id: 'price_1Sr4JVANogcZdSR3locOvXlL', // pro yearly
          ...
          object: {
            ...
            items: {
              data: [
                {
                  price: {
                    id: 'price_1Sr4JVANogcZdSR3locOvXlL',
                    recurring: { interval: 'year', interval_count: 1 },
                  },
                },
              ],
      ...
      if (table === 'profiles') {
        return {
          ...
          update: vi.fn((data: any) => {
            ...
            expect(data.subscription_expires_at).toBeNull();
            return { eq: vi.fn().mockReturnValue({ error: null }) };
          }),
        };
      }
          ...
          object: {
            ...
            items: {
              data: [
                {
                  price: {
                    id: 'price_1Sr4E7ANogcZdSR3Dwu2aPbV',
                  },
                },
              ],
            },
  ...
  it('handles unknown event type gracefully (200)', async () => {
    const stripeModule = await import('stripe');
    (stripeModule as any).__mockSubscriptionsRetrieve.mockResolvedValue(buildSubscription());
  ...
  it('returns 429 after rate limit exceeded', async () => {
    const stripeModule = await import('stripe');
    (stripeModule as any).__mockConstructEvent.mockReturnValue(JSON.parse(fakeEventData));
    const { __mockSupabase } = await import('@supabase/supabase-js');
    (__mockSupabase.from as any).mockImplementation((table: string) => {
      if (table === 'stripe_webhook_events') {
        return {
    ...
    // Make 20 requests (rate limit is 20/min)
    for (let i = 0; i < 20; i++) {
      const evt = makeEvent({
        body: JSON.stringify({
          id: `evt_test_rate_${i}`,
          type: 'checkout.session.completed',
          data: { object: { id: `cs_rate_${i}`, customer: 'cus_mocked_123', subscription: 'sub_mocked_001', client_reference_id: 'user-123' } },
        }),
      });
    ...
    const evt = makeEvent({
      body: JSON.stringify({
        ...
        type: 'checkout.session.completed',
        data: { object: { id: 'cs_rate_limit', customer: 'cus_mocked_123', subscription: 'sub_mocked_001', client_reference_id: 'user-123' } },
      }),
    });
